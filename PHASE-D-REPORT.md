...
that fallback) and `30809133986` (that exclusion decided by offset rather than
by name, i.e. what is in the tree). Each was a
...
10. ~~**The rewritten D3 guard has not itself run in CI yet.**~~ **Closed** by
    run `30809133986`: the version now in the tree ran on a real bump on a
