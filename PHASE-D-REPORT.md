...
(the string table located through `PT_DYNAMIC`) and `30810940553`
(runtime-versioned spellings scoped to the composing object, i.e. what is in
the tree). Each was a
...
10. ~~**The rewritten D3 guard has not itself run in CI yet.**~~ **Closed** by
    run `30810940553`: the version now in the tree ran on a real bump on a
